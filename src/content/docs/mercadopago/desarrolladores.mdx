...
Tambien es posible que necesites realizar una acción adicional luego de que una suscripción sea cancelada, por ejemplo para modificar el estado de la orden.

```php
add_action( 'wc_mercadopago_plus_order_subscription_cancelled', 'crplugins_order_subscription_cancelled', 10, 1 );
function crplugins_order_subscription_cancelled( $order_id ) {
