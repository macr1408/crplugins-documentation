...
```php
add_filter( 'wc_afip_automatic_processor_should_process_order', 'crplugins_automatic_processor_should_process_order', 10, 4 );
function crplugins_automatic_processor_should_process_order( $should_process, $order, $status_from, $status_to ) {
...
```php
add_filter( 'wc_afip_should_process_order', 'crplugins_should_process_order', 10, 3 );
function crplugins_should_process_order( $should_process, $order, $customer_details ) {
	return $order->get_payment_method() === 'bacs';
}
```

### Facturar siempre a consumidor final

```php
add_filter( 'wc_afip_customer_before_process', 'crplugins_always_unidentified_consumer', 10, 3 );
function crplugins_always_unidentified_consumer( $customer, $items, $order ) {
	$customer->set_first_name( '' );
	$customer->set_last_name( '' );
	$customer->set_condition( 'Consumidor Final' );
	$customer->get_address()->set_full_address( '' );
	$customer->get_document()->set_type( 96 );
	$customer->get_document()->set_number( '' );
