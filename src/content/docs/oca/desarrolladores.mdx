...
Adicionalmente ofrecemos un hook para modificar el nombre del medio de envío en el checkout:

```php
add_filter( 'wc_oca_shipping_label', 'crplugins_custom_shipping_method_label', 10, 6 );
function crplugins_custom_shipping_method_label( $shipping_label, $shipping_code, $final_price, $customer_price, $delivery_days, $selected_branch ) {
	$existing_label = 'Envío a Domicilio ({{oca_tiempo_entrega}} {{dias}})';
	$new_label      = 'Express shipping with OCA ({{oca_tiempo_entrega}} {{dias}})';

	if ( $existing_label === $shipping_label ) {
		return $new_label;
	}
...
|wc_oca_shipping_label|filter|$shipping_label, $shipping_code, $final_price, $customer_price, $delivery_days, $selected_branch|Permite modificar el nombre del medio de envío en el checkout|
